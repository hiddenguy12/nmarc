...
router.post('/send', async function (req: Request, res: Response): Promise<any> {
    try {
        if (!req.authSession || !req.authSession?.value) {
            res.status(401).json({
                success: false,
        ...
        const { recipientId } = await createInitialMessageSchema.parseAsync(req.body);
...
router.put('/request/:requestId/accept', async function (req: Request, res: Response): Promise<any> {
    try {
...
router.put('/request/:requestId/reject', async function (req: Request, res: Response): Promise<any> {
    try {
        const { requestId } = req.params;
...
router.put('/request/:requestId/withdraw', async function (req: Request, res: Response): Promise<any> {
    try {
        const { requestId } = req.params;
